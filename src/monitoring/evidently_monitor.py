...
class AbsenteeismMonitor:
    ...
    def check_performance(self, current_data_path, 
                         predictions_column='prediction',
                         actual_column='Absenteeism_time_in_hours_binary'):
        """Check model performance with Accuracy and F1 score."""
        current_data = pd.read_csv(current_data_path)
        timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        ...
        if actual_column not in current_data.columns:
            print(f"Error: Column '{actual_column}' not found")
            return {'status': 'error', 'message': 'Missing ground truth labels'}
        
        if predictions_column not in current_data.columns:
            print(f"Error: Column '{predictions_column}' not found")
            return {'status': 'error', 'message': 'Missing predictions'}
        
        # Calculate metrics using sklearn
        from sklearn.metrics import accuracy_score, f1_score as f1_metric
        
        y_true = current_data[actual_column]
        y_pred = current_data[predictions_column]
        
        accuracy = accuracy_score(y_true, y_pred)
        f1_score = f1_metric(y_true, y_pred)
        ...
        if accuracy >= self.thresholds['accuracy_target'] and f1_score >= self.thresholds['f1_target']:
            status = 'passed'
        else:
            status = 'warning'
        ...
        # Prepare data for Evidently report
        test_data = current_data.copy()
        test_data['target'] = test_data[actual_column]
        test_data['prediction'] = test_data[predictions_column]
        
        # Generate Evidently HTML report
        report = Report(metrics=[ClassificationPreset()])
        report.run(reference_data=self.reference_data, current_data=test_data)
...
if __name__ == "__main__":    
    ...
    monitor.check_performance('data/raw/work_absenteeism_modified.csv')
